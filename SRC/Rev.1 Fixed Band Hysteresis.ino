 ...
 * - Vent servo control (closed / half open / full open)
 ...
 * - Integrated iFrame for esp32cam LIVE monitoring
 * 
 ...
 * - 2x 2.2KOhms Resistor for DS12B20 temperature sensors for reducing signal noise if using long cables on VCC and DATA
...
#define DHTTYPE       DHT11 // You Can Use DHT22
...
// Temperature bands for normal modes
const float FULL_OPEN_BAND = 3.0;   // Full open + high fan if ≥ target + 3°C
const float HALF_OPEN_BAND = 2.0;   // Half open + low fan within ±2°C
// ===========================================================
...
void loop() {
  ...
  if (lastBtn == HIGH && btn == LOW) {
    ...
    if (!inSubMenu && menuIndex != 7) {
      ...
      if (!isnan(chamberTemp)) {
        if (chamberTemp >= activeTarget + FULL_OPEN_BAND) updateFan(FAN_HIGH);
        else if (chamberTemp >= activeTarget - HALF_OPEN_BAND && chamberTemp <= activeTarget + HALF_OPEN_BAND) updateFan(FAN_LOW);
        else updateFan(FAN_OFF);
      } else {
        updateFan(FAN_OFF);
      }
  ...
  if (inSubMenu && !isnan(chamberTemp)) {
    ...
    if (intakeFault) updateFan(FAN_HIGH);
      ...
      if (chamberTemp >= activeTarget + FULL_OPEN_BAND) {
        startOpenVent(false);
        updateFan(FAN_HIGH);
      } else if (chamberTemp >= activeTarget - HALF_OPEN_BAND && chamberTemp <= activeTarget + HALF_OPEN_BAND) {
        startOpenVent(true);
        updateFan(FAN_LOW);
      } else if (chamberTemp < activeTarget - HALF_OPEN_BAND) {
        startCloseVent();
        updateFan(FAN_OFF);
      }
    }
