/*
 * Chamber Master - 3D Printer Enclosure Controller - Version 2.9-beta (Klipper Integration)
 * 
 ...
 * 
 * Major Version 2.9-beta Features & Enhancements:
 * 1. Klipper 3D Printer REST API Integration:
 *    - Added `/material` endpoint (GET & POST) to remotely set active material modes
 *      (PLA, ASA, ABS, TPU, PETG, CUSTOM, COOLDOWN) and custom temperatures directly
 *      from Klipper macros or slicer filament start G-code.
 *    - Synchronizes internal state (`inSubMenu = true`, `menuIndex`, `activeMode`, `activeTarget`)
 *      and executes immediate thermal hysteresis evaluation so cooling/venting responds instantly.
 *    - Persists mode and custom target temperatures to ESP32 NVS flash.
 *    - Full CORS support (`Access-Control-Allow-Origin: *`) and JSON responses.
 * 2. Inherited v2.8 True 0-RPM & High-Z Fan Isolation:
 *    - High-level voltage matching on GPIO 33 and High-Z pin isolation across inline diodes.
 * 
...
// Directional hysteresis thresholds (offsets from activeTarget in °C)
// Fan ONLY engages when chamber temp EXCEEDS target, never below.
// Dead-band widths: HALF = 1.5°C, FULL = 1.0°C (prevents servo chatter)
const float HYSTERESIS_TO_HALF   =  1.0f;  // CLOSED → HALF when chamberTemp > target + 1.0°C
const float HYSTERESIS_TO_CLOSED = -0.5f;  // HALF → CLOSED when chamberTemp < target - 0.5°C
const float HYSTERESIS_TO_FULL   =  3.0f;  // HALF → FULL when chamberTemp > target + 3.0°C
const float HYSTERESIS_FROM_FULL =  2.0f;  // FULL → HALF when chamberTemp <= target + 2.0°C
// ===========================================================
...
// ==================== MATERIAL / KLIPPER WEB HANDLER ====================
// Klipper integration idea proposed by Richard Kennett (https://github.com/richard-kennett - Issue #3)
void handleMaterial() {
  ...
  if (!server.hasArg("material")) {
    server.send(400, "application/json", "{\"error\":\"Missing 'material' parameter\"}");
    return;
  }

  String mat = server.arg("material");
  mat.trim();

  int targetIndex = -1;
  for (int i = 0; i < MENU_LEN; i++) {
    if (mat.equalsIgnoreCase(menuItems[i])) {
      targetIndex = i;
      break;
    }
  }

  if (targetIndex < 0 || targetIndex == 7) { // Index 7 is "QR CODE", not a thermal mode
    server.send(400, "application/json", "{\"error\":\"Unknown material or invalid mode\"}");
    return;
  }

  // Handle COOLDOWN mode request
  if (targetIndex == 6) {
    activeMode = 6;
    ...
    displayNeedsUpdate = true;
    server.send(200, "application/json", "{\"status\":\"OK\",\"mode\":\"COOLDOWN\"}");
    return;
  }

  // Handle CUSTOM target temperature if specified
  if (targetIndex == 5) {
    String tempStr = "";
    if (server.hasArg("temperature")) {
      tempStr = server.arg("temperature");
    } else if (server.hasArg("temp")) {
      tempStr = server.arg("temp");
    } else if (server.hasArg("target")) {
      tempStr = server.arg("target");
    }
    ...
    if (tempStr.length() > 0) {
      ...
      if (t >= 0.0f && t <= 120.0f) {
        ...
        server.send(400, "application/json", "{\"error\":\"Temperature out of range (0.0 - 120.0 C)\"}");
        return;
      }
    }
  }

  // Set active material mode (0 to 5)
  activeMode = targetIndex;
  menuIndex = activeMode;
  inSubMenu = true;
  activeTarget = (activeMode == 5) ? customTarget : menuTargets[activeMode];
  prefs.putInt("activeMode", activeMode);

  // Clear cooldown state if transitioning out of cooldown
  cooldownEstSeconds = -1;
  cooldownProgress = 0.0f;

  // Immediate thermal evaluation so fans and vent react without delay
  ...
  String resp = "{\"status\":\"OK\",\"mode\":\"";
  resp += menuItems[activeMode];
  resp += "\",\"targetTemp\":";
  resp += String(activeTarget, 1);
  resp += "}";
...
void setupWiFiAndServer() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    server.on("/material", HTTP_GET, handleMaterial);
    server.on("/material", HTTP_POST, handleMaterial);
...
void setup() {
  ...
  Serial.begin(115200);
  delay(50);
  Serial.println("Starting Chamber Master v2.9-beta (Klipper Integration)...");
  #endif
  ensureOLED();
