/*
 * Chamber Master - 3D Printer Enclosure Controller - Version 2.8 (Single File Production Sketch)
 * 
 ...
 * 
 * Major Version 2.8 Fixes & Features:
 * 1. HIGH-Level Voltage Matching on FAN_OFF (Compatible with inline diodes):
 *    - On FAN_OFF, GPIO 33 is now driven HIGH (3.3V). Standard 4-pin fans have an internal
 *      pull-up to 3.3V/5V on the PWM line. Driving GPIO 33 HIGH eliminates the voltage
 *      potential difference (3.3V - 3.3V = 0V) across the wire and inline diodes (e.g., 1N5408G).
 *      Zero voltage potential prevents forward diode conduction and parasitic ground leakage,
 *      stopping the fan 100% DEAD (true 0 RPM) regardless of heat-shrunk diodes or fan wiring.
...
void setup() {
  ...
  Serial.begin(115200);
  delay(50);
  Serial.println("Starting Chamber Master v2.8...");
  #endif
  ensureOLED();
