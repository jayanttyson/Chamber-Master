/*
 * Chamber Master - 3D Printer Enclosure Controller - Version 2.7 (Single File Production Sketch)
 * 
 ...
 * 
 * Major Version 2.7 Fixes & Features:
 * 1. True 0-RPM Parasitic Grounding Kill:
 *    - On FAN_OFF, PWM pin (GPIO 33) and Tachometer pin (GPIO 19) are now detached
 *      and set to INPUT (Floating High-Z mode). When the low-side transistor cuts GND,
 *      standard 4-pin PC fans attempt to sink internal pull-up current through the 0V PWM
 *      or Tach line to ESP32 GND, causing slow spinning (~100-300 RPM). Setting GPIO 33
 *      and GPIO 19 to INPUT High-Z replicates physically cutting the control wire,
 *      eliminating ALL parasitic ground current and stopping the fan dead (0 RPM).
 * 2. Automatic Re-Attachment:
 *    - On FAN_ON, the 25 kHz PWM timer and Tachometer interrupt are automatically re-attached,
 *      restoring full 0-100% PWM speed control and RPM sensing seamlessly.
 * 
...
void setFanDuty(uint8_t d, bool allowBelow20 = false) {
  ...
  if (allowBelow20 && d < NORMAL_MIN_DUTY) {
    ...
    // 2. Eliminate Parasitic Grounding:
    // Detach PWM pin and set to INPUT (Floating High-Z mode - EXACTLY like cutting the wire!)
...
void setup() {
  ...
  Serial.begin(115200);
  delay(50);
  Serial.println("Starting Chamber Master v2.7...");
  #endif
  ensureOLED();
