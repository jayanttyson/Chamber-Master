/*
 * Chamber Master - 3D Printer Enclosure Controller - Version 2.6 (Single File Production Sketch)
 * 
 ...
 * 
 * Major Version 2.6 Fixes & Features:
 * 1. Fan Hard Kill Desync Fix:
 *    - updateFan(FAN_OFF) now ALWAYS forces FAN_POWER_PIN LOW, even if fanSpeed
 *      is already cached as FAN_OFF. Cooldown mode's direct setFanDuty() calls
 *      previously set FAN_POWER_PIN HIGH without updating fanSpeed, leaving the
 *      transistor ON after exiting cooldown. The fan would spin at Intel failsafe
 *      ~20% minimum indefinitely on main menu or in profiles below target.
 * 2. Hysteresis Re-Calibration:
 *    - CRITICAL: Previous thresholds engaged the fan 1°C BELOW target temperature.
 *      The fan exhausted hot air before the chamber reached setpoint, causing the
 *      enclosure to run ~1.5°C below target permanently (e.g., PLA at 28.5°C
 *      instead of 30°C). New thresholds:
 *        HALF engage:  target + 1.0°C  (was target - 1.0°C)
 *        HALF release: target - 0.5°C  (was target - 2.0°C)
 *        FULL engage:  target + 3.0°C  (was target + 2.0°C)
 *        FULL release: target + 2.0°C  (was target + 1.0°C)
 *      Chamber now holds ±0.2°C of target with 1.5°C anti-chatter dead-band.
 * 3. Explicit FAN_OFF in VENT_CLOSED State:
 *    - Normal preset thermal loop now explicitly calls updateFan(FAN_OFF) when
 *      vent is closed and chamber temp is below target, preventing any residual
 *      fan state from persisting after mode switches.
 * 
...
void setup() {
  ...
  Serial.begin(115200);
  delay(50);
  Serial.println("Starting Chamber Master v2.6...");
  #endif
  ensureOLED();
