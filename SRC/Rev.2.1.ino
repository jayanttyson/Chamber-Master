...
long cooldownEstSeconds = 0;           // Now properly handled: -1 = calculating, 0 = cooled
...
void drawCooldownScreen() {
  ...
  // FIXED: Now matches web dashboard exactly
...
void handleStartCooldown() {
  if (server.method() == HTTP_POST || server.method() == HTTP_GET) {
    ...
    cooldownEstSeconds = -1;  // Show "Calculating..." at start
